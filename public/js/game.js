...
const isPvpMode = (gameId === 'pvp_arena'); // Главная проверка режима
...
let currentSlot = 1;
let hasWeapon = isPvpMode; // Оружие только в ПВП
...
fetch('/api/me')
    .then(r => r.json())
    .then(d => { currentUser = d.user; initGame(); })
    .catch(() => initGame());
...
function initGame() {
    ...
    scene.background = new THREE.Color(0x87CEEB); // Сделал небо голубым, приятнее
    ...
    // Если это Паркур, скрываем инвентарь и HP
    if (!isPvpMode) {
        const invBar = document.getElementById('inventory-bar');
        if (invBar) invBar.style.display = 'none';
        
        const hpBar = document.getElementById('health-container');
        if (hpBar) hpBar.style.display = 'none';
        
        const attackBtn = document.getElementById('btn-action');
...
// =================== МОДЕЛИРОВАНИЕ (UPDATE: АКСЕССУАРЫ) ===================

function createFaceTexture() {
    const canvas = document.createElement('canvas'); canvas.width=128; canvas.height=128;
    const ctx = canvas.getContext('2d');
    ctx.fillStyle='#ffcc99'; ctx.fillRect(0,0,128,128); // Цвет кожи
...
// !!! ГЛАВНОЕ ОБНОВЛЕНИЕ: ОДЕЖДА И ШЛЯПА !!!
function createPlayerModel() {
    ...
    // --- МАТЕРИАЛЫ ---
    const skinMat = new THREE.MeshPhongMaterial({ color: 0xffcc99 }); // Кожа
    const sweaterMat = new THREE.MeshPhongMaterial({ color: 0xcc4444 }); // Красный свитер
    const jeansMat = new THREE.MeshPhongMaterial({ color: 0x3366cc });   // Синие джинсы
    const hatMat = new THREE.MeshPhongMaterial({ color: 0x111111 });     // Черная шляпа
    const faceMat = new THREE.MeshBasicMaterial({ map: createFaceTexture() });

    // --- 1. ТЕЛО (Свитер) ---
    // Размеры: ширина 4, высота 5, глубина 2.5
    ...
    // --- 2. ГОЛОВА ---
    // Размеры: 3.5 куб
    // Массив материалов для лица (передняя грань отличается)
    ...
    // --- АКСЕССУАР: ЦИЛИНДР (TOP HAT) ---
    // Поля шляпы (Brim)
    const brim = new THREE.Mesh(new THREE.CylinderGeometry(2.5, 2.5, 0.2, 12), hatMat);
    brim.position.y = 1.8; // Чуть выше центра головы (1.75 - это верх головы)
    head.add(brim); // Добавляем к голове, чтобы вращалась вместе с ней
    ...
    // Функция для создания конечностей
    function limb(x, y, mat, w=1.5, h=5, d=1.5) {
        const p = new THREE.Object3D(); 
        p.position.set(x, y, 0);
        const m = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), mat); 
        m.position.y = -h / 2; // Смещаем центр вращения вверх
        m.castShadow = true;
        p.add(m); 
        model.add(p); 
    ...
    // --- 3. НОГИ (Джинсы) ---
    const lL = limb(-1.2, 5, jeansMat); 
    const lR = limb(1.2, 5, jeansMat);

    // --- 4. РУКИ (Свитер) ---
    const aL = limb(-2.8, 9.5, sweaterMat); 
    const aR = limb(2.8, 9.5, sweaterMat);

    let sword = null;
    // Добавляем меч, если это PVP режим
    if (isPvpMode) {
        sword = buildCoolSword();
        sword.visible = true;
        aR.pivot.add(sword);
    }
    ...
    return { 
        group, 
        model, 
        limbs: { legL:lL.pivot, legR:lR.pivot, armL:aL.pivot, armR:aR.pivot, sword } 
    };
}
...
function addOtherPlayer(info) {
    ...
    sprite.position.set(0, 17, 0); // Чуть выше из-за шляпы
    ...
    otherPlayers[info.id] = { 
        ...
        targetRot: 0, isMoving: false, hasWeapon: isPvpMode
    };
}
...
function loadLevel(id) {
    colliders = []; // Очищаем старые коллайдеры
    ...
    if (id === 'parkour_1') {
        // === НОРМАЛЬНЫЙ ПАРКУР (OBBY) ===
        // Старт
        box(0, -5, 0, 20, 2, 20, 0x444444); 
        // Платформы
        box(0, 0, 20, 6, 1, 6, 0xffaa00);
        box(0, 2, 35, 6, 1, 6, 0xffaa00);
        box(0, 4, 50, 6, 1, 6, 0xffaa00);
        // Балка
        box(0, 4, 70, 2, 1, 25, 0x333333); 
        // Лестница
        box(0, 8, 90, 8, 1, 8, 0x555555); 
        box(10, 11, 90, 4, 1, 4, 0x00aaff);
        box(18, 14, 90, 4, 1, 4, 0x00aaff);
        box(26, 17, 90, 4, 1, 4, 0x00aaff);
        // Финишный прыжок
        ...
        goal.position.set(45, 22, 90);
        scene.add(goal);

    } else { 
        // === PVP ARENA ===
        // Пол
        box(0, -5, 0, 120, 2, 120, 0x333333); 
        // Стены
        box(0, 5, -60, 120, 20, 2, 0x222222); 
        box(0, 5, 60, 120, 20, 2, 0x222222);
        box(-60, 5, 0, 2, 20, 120, 0x222222); 
        box(60, 5, 0, 2, 20, 120, 0x222222);
        
        // Укрытия
        box(-30, 0, -30, 10, 8, 10, 0x884444); 
        box(30, 0, 30, 10, 8, 10, 0x884444); 
        box(0, 2, 0, 20, 4, 20, 0x666666);
    }
}
...
function resolveCollision(pos, radius, height) {
    ...
    // Немного уменьшаем радиус коллизии, чтобы не застревать в стенах
    box.set(
        new THREE.Vector3(pos.x - radius * 0.8, pos.y, pos.z - radius * 0.8), 
        new THREE.Vector3(pos.x + radius * 0.8, pos.y + height, pos.z + radius * 0.8)
    );
...
function breakPlayerApart(pos) {
    // Части тела при смерти (цвета соответствуют одежде)
    const parts = [
        { s:[3.5,3.5,3.5], p:[0,6.5,0], c:0xffcc99 }, // Голова
        { s:[4,5,2.5], p:[0,2.5,0], c:0xcc4444 },     // Свитер
        { s:[1.5,5,1.5], p:[-1.2,0,0], c:0x3366cc },  // Нога Л (Джинсы)
        { s:[1.5,5,1.5], p:[1.2,0,0], c:0x3366cc },   // Нога П (Джинсы)
        { s:[1.5,5,1.5], p:[-2.8,4.5,0], c:0xcc4444 },// Рука Л (Свитер)
        { s:[1.5,5,1.5], p:[2.8,4.5,0], c:0xcc4444 }  // Рука П (Свитер)
    ];
    parts.forEach(p => {
...
function setupInventoryUI() {
    if (!isPvpMode) return; // В паркуре инвентарь не нужен
...
function selectSlot(num) {
    ...
    hasWeapon = (num === 1);
    // Проверка на существование меча перед обращением к нему
...
function setupNetwork() {
    ...
    socket.on('playerMoved', p => {
        if(otherPlayers[p.id]) {
            ...
            if(p.action === 'equip_change') { 
                pl.hasWeapon = p.weapon; 
                if(pl.limbs.sword) pl.limbs.sword.visible = p.weapon; 
            }
        }
    });
    ...
    socket.on('playerDied', data => {
        if(data.id === socket.id) {
            ...
            breakPlayerApart(playerGroup.position);
        } else if(otherPlayers[data.id]) {
            otherPlayers[data.id].mesh.visible = false;
            breakPlayerApart(otherPlayers[data.id].mesh.position);
        }
    });
...
function playAttackAnimation(limbSet) {
    if (!limbSet.sword) return; // Нет меча - нет анимации
...
function animateLimbs(limbSet, time, isMoving, isJumping, hasSwordEquipped) {
    ...
    // Руки
    if (isJumping) {
        limbSet.armL.rotation.x = -Math.PI; 
        limbSet.armR.rotation.x = -Math.PI;
    } else {
        limbSet.armL.rotation.x = isMoving ? -Math.sin(time * 0.015) * 0.8 : 0;
        
        // Правая рука (зависит от оружия)
        if (hasSwordEquipped && limbSet.sword) {
